...
class HttpRequest {
  ...
  static final BaseOptions _options = BaseOptions(
    baseUrl: 'http://192.168.2.12:4230',
  ...
  static Future<T> _request<T>(String path,
    ...
    headers ??= {};
    headers['access_token'] = await SharedPreferencesUtils.getStorage(
        SharedPreferencesUtils.TOKEN_KEY);
    headers['auth_password'] = await SharedPreferencesUtils.getStorage(
        SharedPreferencesUtils.PASSWORD_KEY);
    LogUtil.v(data, tag: '发送的数据为：');
    LogUtil.v(path, tag: '请求路径：');
  ...
  static Future<T> request<T>(String path,
      {method, Map? params, data, Map<String, dynamic>? headers}) {
    return _request(path,
        method: method, params: params, data: data, headers: headers);
  }
}
