...
import 'package:my_fist_flutter/http/api/UserApi.dart';
...
class UserService {
  ...
  static Future<UserInfo> getUserInfoByLocal() async {
    var userInfo= await SharedPreferencesUtils.getStorage(SharedPreferencesUtils.USER_KEY);
    if (userInfo == null ) {
  ...
  static Future<UserInfo> login(String account, String password) async {
    ...
    SharedPreferencesUtils.setStorage(SharedPreferencesUtils.PASSWORD_KEY, authPassword);
    ...
    userInfo.isLogin = '1';
    SharedPreferencesUtils.setStorage(SharedPreferencesUtils.USER_KEY, userInfo.toJson());
